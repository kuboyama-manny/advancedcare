// Patient Controller
angular.module('app').controller('PatientCtrl', ['$scope', '$state', '$stateParams', '$window', '$filter', 'APIService', 'TimeFormatService',
  function($scope, $state, $stateParams, $window, $filter, APIService, TimeFormatService) {
    $scope.disp_me_name = "";
    ...
    $scope.loadData = function() {
      ...
      APIService.get('/patient?id=' + $scope.patientId).then(function(response) {
        ...
        $scope.patient.notes.forEach(function(note, index, array) {
          $scope.patient.notes[index].created_at = TimeFormatService.format(note.created_at);
        })

        $scope.diagnosisPanel = false;
        $scope.diagnosisInEdit = false;
        if ($scope.patient.emar) {
    ...
    $scope.get_content = function(medication) {
      ...
      $scope.med_schedules.map(function(item) {
        ...
        if (medication.drug_product_id == item.drug_product_id && item.reason != "") {
          ...
          //ret_str = ret_str + ("0" + (scheduled_at.getHours())).slice(-2) + ("0" + (scheduled_at.getMinutes())).slice(-2)+ " " + item.reason + "<br>" ;
        }
      });
    ...
    $scope.click_createMedSchedule = function(column, medication) {
      ...
      if (f_flag == 1) {
        ...
        $scope.schedule_date = now.getFullYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate();
        ...
        $scope.schedule_date = scheduled_at.getFullYear() + "-" + (scheduled_at.getMonth() + 1) + "-" + scheduled_at.getDate();

        $scope.new_med_schedule_notes = $scope.cur_med_schedule.reason;

        $scope.selectedRepeatsModel = $scope.selectedRepeatsModel_arr[$scope.cur_med_schedule.repeat_by - 1];
    ...
    $scope.createNewMedSchedule = function() {
      ...
      if ($scope.new_flag == 1) {
        ...
        var new_med_schedule = {
          ...
          repeat_by: $scope.selectedRepeatsModel.id,
    ...
    $scope.addAllergy = function() {
      if ($scope.allergy.name === undefined || $scope.allergy.name === '') return;
      $scope.patient.allergies.push($scope.allergy);
      $scope.allergy = {};
    };

    $scope.newAllergy = function() {
      $scope.allergy = angular.copy({});
      $scope.allergyInEdit = true;
      $scope.allergyPanel = true;
      $scope.allergySubmitted = false;
    };

    $scope.cancelAllergy = function() {
      $scope.allergyPanel = false;
    };

    $scope.saveAllergy = function(isValid) {
      $scope.allergySubmitted = true;
      if (!isValid) {
        $scope.saveFailed = true;
        $scope.error_m = 'required_fields_missing';
        return false;
    ...
    $scope.updateAvatar = function() {
      ...
      r.onloadend = function(e) {
        ...
        APIService.patch_b64('/patient/avatar?id=' + $scope.patientId, data).then(function(response) {
          $scope.helpers.uiBlocks('.block', 'state_normal');
          if (response.s === 'f' && response.m !== '') {
            alert('Upload failed! The avatar image is too large.');
          } else {
            $scope.patient.avatar_url = APIService.API_URL + '/patient/avatar?id=' + $scope.patient.id + '&token=' + encodeURIComponent($window.localStorage.token) + '&t=' + new Date().getTime();
