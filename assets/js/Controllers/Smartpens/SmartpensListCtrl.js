// Facility Smartpens List Controller
angular.module('app').controller('SmartpensListCtrl', ['$scope', '$stateParams', 'APIService',
    function ($scope, $stateParams, APIService) {
        ...
        $scope.loadData = function () {
            ...
            var url = request === ''? '/facility/smartpen?' : '/facility/smartpen?q=' + request + "&";
            ...
            APIService.get(url + 'count=' + $scope.page_size + '&page=' + $scope.current_page).then(function (response) {
                ...
                $scope.smartpens = response.result.smartpens;
                $scope.facilities = response.result.facilities;

                $scope.smartpens.forEach(function (smartpen, index, array) {
                    smartpen.facility = $.grep($scope.facilities, function (e) {
                        return e.id == smartpen.facility_id;
                    })[0];
                });
